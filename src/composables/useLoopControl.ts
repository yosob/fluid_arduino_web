...
export function useLoopControl() {
   ...
   * 等待 startLoop 时统一发送
   */
  async function addLoopStep(channel: number, pumpType: PumpType, pwm: number, time: number): Promise<boolean> {
   ...
   * 删除时序指令（仅从 UI 列表中删除）
   */
  function removeLoopStep(id: number): void {
   ...
   * 清空时序表（本地和下位机都清空）
   */
  async function clearLoop(): Promise<boolean> {
   ...
   * 3. 发送LOOP_START命令
   */
  async function startLoop(loopCount = 0): Promise<boolean> {
    const sequence = loopStore.sequence as any[]
    ...
    for (let i = 0; i < sequence.length; i++) {
      const step = sequence[i] as { channel: number; pumpType: PumpType; pwm: number; time: number }
   ...
   */
  async function stopLoop(): Promise<boolean> {
   ...
   * 暂停循环执行
   */
  async function pauseLoop(): Promise<boolean> {
   ...
   * 继续循环执行
   */
  async function resumeLoop(): Promise<boolean> {
   ...
   * 获取泵类型名称
   */
  function getPumpTypeName(pumpType: PumpType): string {
