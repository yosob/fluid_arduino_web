...
import { ref } from 'vue'
import type { LoopChannelStatus } from '@/types'
...
export const useLoopStore = defineStore('loop', () => {
  ...
  const totalLoops = ref<number>(0) // 0=无限循环

  // 时序表（统一存储，包含channel字段）
  const sequence = ref<LoopStep[]>([])
   ...
   * 添加时序指令
   */
  function addStep(step: Omit<LoopStep, 'id'>): void {
    sequence.value.push({
      id: Date.now(),
      ...step
    })
   ...
   */
  function updateStatus(loopStatusData: LoopStatus): void {
